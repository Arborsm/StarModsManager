...
public class OnlineMod
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if ((string.IsNullOrEmpty(PicUrl) || (refresh && !PicUrl.Contains("http")) || PicUrl == "https://www.nexusmods.com/assets/images/default/noimage.svg") && !string.IsNullOrEmpty(Url) && !Url.Contains("???"))
        {
            var nexusMod = await NexusMod.CreateAsync(Url, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            await using (var fileStream = new FileStream(cachePath, FileMode.Create, FileAccess.Write, FileShare.Read, 4096, true))
            {
