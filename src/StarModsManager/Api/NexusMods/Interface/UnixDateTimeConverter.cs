    ...
    public override DateTimeOffset Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        else
        {
            throw new JsonException(
                $"Unexpected token parsing date. Expected Number or String, got {reader.TokenType}.");
        }

        if (unixTime < 0)
            throw new JsonException(
                "Cannot convert value that is before Unix epoch of 00:00:00 UTC on 1 January 1970.");
