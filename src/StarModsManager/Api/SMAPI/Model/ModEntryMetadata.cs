...
public class ModEntryMetadata
{
    ...
    public ModEntryVersion Main { get; set; } = new();

    /// <summary>The latest unofficial version, if newer than <see cref="Main"/> and <see cref="Optional"/>.</summary>
    public ModEntryVersion Unofficial { get; set; } = new();

    public string CustomUrl { get; set; } = string.Empty;

    [JsonConverter(typeof(JsonStringEnumConverter<WikiCompatibilityStatus>))]
